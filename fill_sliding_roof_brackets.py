...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="汇总工作簿；不传时读取脚本同目录下的 Summary for Sliding Roof Brackets.xlsm")
    ...
    summary = (args.summary or (script_dir / "Summary for Sliding Roof Brackets.xlsm")).resolve()
    if not summary.is_file():
        parser.error(f"找不到汇总工作簿: {summary}\n请把脚本与 Summary for Sliding Roof Brackets.xlsm 放在同一目录。")
